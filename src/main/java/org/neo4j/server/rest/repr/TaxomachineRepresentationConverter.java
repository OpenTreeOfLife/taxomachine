...
public class TaxomachineRepresentationConverter extends OTRepresentationConverter {

	/**
	 * Return a serialization of the passed in data, which is created by first converting `data` to a Representation
	 * object (which can then be serialized). An attempts is made to determine the best approach to serializing `data`
	 * by examining the type of `data` and then calling a specialized converter method intended to deal with that type
	 * of data. This works for primitives, simple container types that implement Iterable, and objects with explicit
	 * conversion methods defined, but will fail on complex datatypes (e.g. classes with instance variables) that do not
	 * have explicitly defined conversion methods. New conversion methods can be defined easily; see conversion methods
	 * for TNRSResults and ContextResult for examples.
	 * 
	 * @param data
	 * @return serialized
	 */
	 ...
	 * @return
	 * 
	 * public static MappingRepresentation getMapRepresentation(Map data) { return
	 * GeneralizedMappingRepresentation.getMapRepresentation(data); }
	 */
	...
	/*
	 * static Representation getIteratorRepresentation(Iterator data) { final FirstItemIterable<Representation> results
	 * = new FirstItemIterable<Representation>( new IteratorWrapper<Representation, Object>(data) {
	 * 
	 * @Override protected Representation underlyingObjectToObject(Object value) { if (value instanceof Iterable) {
	 * FirstItemIterable<Representation> nested = convertValuesToRepresentations((Iterable) value); return new
	 * ListRepresentation(getType(nested), nested); } else { return getSingleRepresentation(value); } } } ); return new
	 * ListRepresentation(getType(results), results); }
	 */
	...
	/**
	 * Infer the type of the objects contained by the Iterable `representationIter`, or if `representationIter` has no
	 * elements in its iterator, then return the type of `representationIter` itself. Used by other converter methods to
	 * sniff the datatypes of elements in containers so that the appropriate converter methods can be called.
	 * 
	 * @param representationIter
	 * 
	 * @return type of elements in
	 * 
	 * static RepresentationType getType(FirstItemIterable<Representation> representationIter) { Representation
	 * representation = representationIter.getFirst(); if (representation == null) return RepresentationType.STRING;
	 ...
	 * RepresentationConverter serialization methods, so all objects to be serialized (including primitives) must be
	 * converted to a Representation; this method provides that functionality.
	 * 
	 * @param data
	 * 
