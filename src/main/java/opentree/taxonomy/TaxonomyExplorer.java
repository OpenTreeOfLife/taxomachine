...
public class TaxonomyExplorer extends TaxonomyBase {
     ...
     * 
     * @param clade_name
     *            - the name of the internal node in taxNodeIndex that will be
    ...
    public void exportGraphForClade(String clade_name, String out_filepath) {
        ...
        // TraversalDescription CHILDOF_TRAVERSAL =
        // Traversal.description().relationships(RelTypes.TAXCHILDOF,Direction.INCOMING
        // );
        ...
        try {
            ...
            int count = 0;
            for (Node nd : firstNode.traverse(Traverser.Order.BREADTH_FIRST, StopEvaluator.END_OF_GRAPH, ReturnableEvaluator.ALL, RelTypes.TAXCHILDOF,
                    Direction.INCOMING)) {
    ...
    /**
     * This essentially uses every relationship and constructs a newick tree
     * (hardcoded to taxtree.tre file)
     * 
     * It would be trivial to only include certain relationship sources @ name
    ...
    /*
     * Given a Taxonomic name (as name), this will attempt to find cycles which
    ...
    /*
     * given a taxonomic name, construct a json object of the graph surrounding
     * that name
     */
    ...
    /**
     * Right now this walks from the life node through the entire graph. When
     * there are conflicts, this prefers the NCBI branches.
     */
